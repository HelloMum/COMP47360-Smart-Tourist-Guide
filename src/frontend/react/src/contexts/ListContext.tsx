...
interface ListContextProps {
  ...
  addItemWithDateCheck: (item: ListItem, onMissingDates: () => void, componentName: string) => void;
  planData: any;
...
type Action =
  ...
  | { type: 'CLEAR_LIST' }
  | { type: 'SET_SELECTED_DATES'; dates: [moment.Moment | null, moment.Moment | null] | null }
...
function reducer(state: typeof initialState, action: Action) {
  switch (action.type) {
    ...
    case 'REMOVE_FROM_LIST': {
      const newListItems = state.listItems.filter(item => item.title !== action.title);
...
export const ListProvider: React.FC<ListProviderProps> = ({ children }) => {
  ...
  const addItemWithDateCheck = (item: ListItem, onMissingDates: () => void, componentName: string) => {
    if (!state.selectedDates || !state.selectedDates[0] || !state.selectedDates[1]) {
      onMissingDates();
    } else {
  ...
  return (
    <ListContext.Provider value={{
      ...
      setSelectedDates: dates => dispatch({ type: 'SET_SELECTED_DATES', dates }),
